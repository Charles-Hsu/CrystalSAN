...
- (id)initWithView:(UIView *)view {
    ...
    //NSLog(@"%s %f %f %f %f", __func__, view.bounds.origin.x, view.bounds.origin.x, view.bounds.size.height, view.bounds.size.width);
    //NSLog(@"%s yOffset=%f", __func__, self.yOffset);
...
- (void)setupLabels {
	...
	detailsLabel.textAlignment = UITextAlignmentLeft;// MBLabelAlignmentCenter;
...
- (void)layoutSubviews {
	...
	if (parent) {
        ...
        //self.backgroundColor = [UIColor greenColor]; // added by Charles Hsu 2013/4/1
        yOffset = 0; // force to 0, modified by Charles Hsu 2013/4/1
        
	}
	CGRect bounds = self.bounds;
    //NSLog(@"%s bounds x=%f y=%f h=%f w=%f", __func__, bounds.origin.x, bounds.origin.y, bounds.size.height, bounds.size.width);
    ...
    //NSLog(@"%s indicatorF x=%f y=%f h=%f w=%f", __func__, indicatorF.origin.x, indicatorF.origin.y, indicatorF.size.height, indicatorF.size.width);
    ...
    //NSLog(@"%s margin=%f", __func__, margin);
    //NSLog(@"%s yOffset=%f", __func__, yOffset);
    //NSLog(@"%s yPos=%f", __func__, yPos);
